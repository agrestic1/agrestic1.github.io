...
# ------ Routers --------
  routers:
    dsm-insec:
      ...
      middlewares:
        - https-redirect # Leitet auf port 443 um, erzwingt also SSL fuer den externen traffik
      service: service-dsm #SSL auch fuer internen traffic (mit DSM's self-signed certificate, daher muss serversTransport: insecureSkipVerify: true sein)
...
```bash
cd /volume1/docker/
mkdir -p crowdsec/data && cd crowdsec
